...
var IBOP = function () {
    ...
    return {
        ...
        init: function () {
            ...
            $(document).on('change', '#genAll', function (e) {
                if($(this).is(':checked')){
                    $('#prop-input').hide();
                }else{
                    $('#prop-input').show();
        ...
        loadProperties: function () {
            Common.showSpinner($form, "Loading Properties...");
            var requestUrl = $form.attr("data-properties-url");
            $.get(requestUrl, {}, function (response) {
                var data = $.map(response.data, function (obj) {
                    obj.name = obj.digitaladdress + '-' + obj.location;
                    return obj;
                });
                if (data.length > 0) {
                    Common.populateDropDown(slBcTypes, data, "id", "name");
