...
var IBOP = function () {
    ...
    return {
        ...
        loadBusinesses: function () {
            Common.showSpinner($form, "Loading Businesses...");
            var requestUrl = $form.attr("data-biz-url");
            $.get(requestUrl, {}, function (response) {
                var data = $.map(response.data, function (obj) {
                    return obj;
                });
                if (data.length > 0) {
                    Common.populateDropDown(slBcTypes, data, "id", "businessName");
