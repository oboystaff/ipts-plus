...
class SupportRequestReportController extends Controller
{
    public function index(Request $request)
    {
        try {
            ...
            if (request()->ajax()) {
                ...
                if ($request->report_type == 1) {
                    ...
                    return datatables()->of($data)
                        ->addIndexColumn()
                        ->editColumn('tax_payer', function (CustomerSupport $customerSupport) {
                            $name = '';
                            $first_name = $customerSupport->citizen->first_name ?? 'N/A';
                            $last_name = $customerSupport->citizen->last_name ?? 'N/A';
                            $name = $first_name . ' ' . $last_name;

                            return $name;
                        })
                        ->editColumn('response', function (CustomerSupport $customerSupport) {
                            return $customerSupport->response ?? 'N/A';
                        })
                        ->editColumn('assembly', function (CustomerSupport $customerSupport) {
                            return $customerSupport->assembly->name ?? 'N/A';
                        })
                        ->editColumn('created_by', function (CustomerSupport $customerSupport) {
                            return $customerSupport->createdBy->name ?? 'N/A';
                        })
                        ->editColumn('created_at', function (CustomerSupport $customerSupport) {
                            return $customerSupport->created_at;
                        })
                        ->make(true);
                } else {
            ...
            return view('reports.support-request-report', compact('assemblies'));
        } catch (\Exception $ex) {
